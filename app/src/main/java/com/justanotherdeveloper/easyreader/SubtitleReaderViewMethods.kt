...
class SubtitleReaderViewMethods(private val activity:SubtitleReaderActivity) {
    ...
    private fun initSeekBar() {
        binding.seekbar.progressDrawable.setColorFilter(Color.BLACK)
        binding.seekbar.thumb.setColorFilter(Color.WHITE)
    ...
    private fun updateBackground(backgroundColor: Int) {
        binding.subtitleReaderParent.setBackgroundColor(backgroundColor)
    }
    ...
    private fun updateFontStyle(fontStyle: Int) {
        binding.subtitleTextView.setFontStyle(activity, fontStyle)
    }

    fun hideUI() {
        if(binding.UIArea.visibility == View.VISIBLE) {
            beginTransition(binding.subtitleReaderParent)
            binding.UIArea.visibility = View.GONE
            binding.toolbar.visibility = View.GONE
        }
    }

    fun showUI() {
        if(binding.UIArea.visibility == View.GONE) {
            beginTransition(binding.subtitleReaderParent)
            binding.UIArea.visibility = View.VISIBLE
            binding.toolbar.visibility = View.VISIBLE
        }
    }

    fun showCurrentSubtitle(showTransition: Boolean = true) {
        if(showTransition && !activity.transitionsDisabled())
            beginTransition(binding.subtitleReaderParent)
        binding.subtitleTextView.text = activity.currentSubtitle
    }
    ...
    private fun showMaxWordsSettingEntry(maxWordsTextView: TextView): BottomSheetDialog {
        ...
        recommendedOption.setOnClickListener {
            val enteredText = activity.subtitleSet.enteredText
