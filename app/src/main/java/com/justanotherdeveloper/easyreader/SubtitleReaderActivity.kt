...
import com.justanotherdeveloper.easyreader.databinding.ActivitySubtitleReaderBinding

class SubtitleReaderActivity : AppCompatActivity(), PopupMenu.OnMenuItemClickListener  {

    private lateinit var binding: ActivitySubtitleReaderBinding
    ...
    private var transitionsDisabled = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        local = TinyDB(this)
        transitionsDisabled = local.getBoolean(transitionsDisabledFilename)
    ...
    private fun initBinding() {
        binding = ActivitySubtitleReaderBinding.inflate(layoutInflater)
        setContentView(binding.root)
    }

    fun transitionsDisabled(): Boolean {
        return transitionsDisabled
    }

    fun getBinding(): ActivitySubtitleReaderBinding {
        return binding
    }
    ...
    private fun enableButtons() {
        ...
        binding.pausePlayButton.isEnabled = true
        binding.nextButton.isEnabled = true
        binding.previousButton.isEnabled = true
        binding.leftScreen.isEnabled = true
        binding.rightScreen.isEnabled = true
        binding.settingsButton.isEnabled = true
        binding.subtitleListButton.isEnabled = true
    ...
    private fun disableButtons() {
        ...
        binding.pausePlayButton.isEnabled = false
        binding.nextButton.isEnabled = false
        binding.previousButton.isEnabled = false
        binding.leftScreen.isEnabled = false
        binding.rightScreen.isEnabled = false
        binding.settingsButton.isEnabled = false
        binding.subtitleListButton.isEnabled = false
    ...
    private fun initListeners() {
        ...
        binding.backArrow.setOnClickListener { initFinishProcess() }

        binding.pausePlayButton.setOnTouchListener { _, _ ->
        ...
        binding.subtitleListButton.setOnClickListener {
            if(player.isPlaying) pauseSubtitleReader()
        ...
        binding.moreButton.setOnClickListener {
            ...
            if(transitionsDisabled)
                popup.menu.removeItem(R.id.disableTransitions)
            else popup.menu.removeItem(R.id.enableTransitions)
            popup.show()
        }
    }
    ...
    private fun initNextButtonListeners() {
        ...
        binding.nextButton.setOnLongClickListener {
            nextButtonLongPressed = true
            startContinuousNextButtonPress()
            true
        }

        binding.nextButton.setOnTouchListener { pView, pEvent ->
    ...
    private fun initPreviousButtonListeners() {
        ...
        binding.previousButton.setOnLongClickListener {
            previousButtonLongPressed = true
            startContinuousPreviousButtonPress()
            true
        }

        binding.previousButton.setOnTouchListener { pView, pEvent ->
            pView.onTouchEvent(pEvent)
            // on touch
            if(pEvent.action == MotionEvent.ACTION_DOWN)
                previousButtonPressed()
    ...
    private fun screenTapped() {
        handler.post {
            when {
                binding.UIArea.visibility == View.VISIBLE -> view.hideUI()
    ...
    private fun initLeftScreenListener() {
        binding.leftScreen.setOnClickListener(object : DoubleClickListener() {
    ...
    private fun initRightScreenListener() {
        binding.rightScreen.setOnClickListener(object : DoubleClickListener() {
    ...
    override fun onMenuItemClick(item: MenuItem?): Boolean {
        return when(item?.itemId) {
            ...
            R.id.disableTransitions -> {
                local.putBoolean(transitionsDisabledFilename, true)
                transitionsDisabled = true
                true
            }
            R.id.enableTransitions -> {
                local.putBoolean(transitionsDisabledFilename, false)
